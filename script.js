...
function renderHomePage() { renderDashboard(); renderLeaderboard(); }
function renderOrdersPage() { renderOrdersList(document.getElementById('ordersList'), state.data.weekOrders); }
function renderArchivePage() { renderOrdersList(document.getElementById('archiveListContainer'), state.data.history.flatMap(h => h.orders)); }
...
function renderDashboard() {
  ...
  const isPriv = user.role !== 'MASTER';
  document.querySelector('#dash-revenue .dashboard-item-value').textContent = formatCurrency(weekStats.revenue);
  document.querySelector('#dash-revenue .dashboard-item-title').textContent = isPriv ? 'Выручка (неделя)' : 'Моя выручка';
...
function renderLeaderboard() {
  ...
  container.innerHTML = `<table class="leaderboard-table"><thead><tr><th>Место</th><th>Мастер</th><th>Выручка</th><th>Заказы</th></tr></thead><tbody>
    ${state.data.leaderboard.map((m, i) => `<tr class="${m.name === state.user.name ? 'is-current-user' : ''}"><td>${i+1}</td><td>${m.name}</td><td>${formatCurrency(m.revenue)}</td><td>${m.ordersCount}</td></tr>`).join('')}
    </tbody></table>`;
}
...
function renderOrdersList(container, orders) {
  ...
  orders.forEach(order => {
    ...
    item.innerHTML = `<div><p class="order-description">${order.carModel}: ${order.description}</p><div class="order-meta">${state.user.role !== 'MASTER' ? `<span><i class="fas fa-user"></i>${order.masterName}</span>` : ''}<span><i class="fas fa-tag"></i>${order.paymentType}</span><span><i class="far fa-calendar-alt"></i>${formatDate(order.createdAt)}</span></div></div><div class="order-amount"><div class="order-amount-value">${formatCurrency(order.amount)}</div><div class="order-actions"><button class="btn btn-secondary btn-sm" data-action="edit-order" data-id="${order.id}"><i class="fas fa-pen"></i></button>${state.user.role !== 'MASTER' ? `<button class="btn btn-danger btn-sm" data-action="delete-order" data-id="${order.id}"><i class="fas fa-trash"></i></button>` : ''}</div></div>`;
...
function openOrderModal(order = null) {
  ...
  modal.querySelector('#order-form').addEventListener('submit', (e) => {
    ...
    if (!isPriv) formData.set('masterName', state.user.name);
...
function openSalaryModal() {
    ...
    modal.className = 'modal-backdrop show';
    const isPriv = state.user.role !== 'MASTER';
    const salaryData = state.data.leaderboard.map(m => ({ ...m, salary: m.revenue * 0.5 }));

    modal.innerHTML = `<div class="modal-content"><div class="modal-header"><h3 class="modal-title">Расчет Зарплаты</h3><button class="modal-close-btn" data-action="close-modal">&times;</button></div><div class="modal-body"><table class="salary-table leaderboard-table"><thead><tr><th>Мастер</th><th>Выручка</th><th>ЗП (50%)</th>${isPriv ? '<th>Премия</th>' : ''}<th>Итог</th></tr></thead><tbody>
      ${salaryData.map(m => `<tr><td>${m.name}</td><td>${formatCurrency(m.revenue)}</td><td>${formatCurrency(m.salary)}</td>${isPriv ? `<td><input type="number" class="form-control" data-master-name="${m.name}" value="0"></td>` : ''}<td class="final-salary">${formatCurrency(m.salary)}</td></tr>`).join('')}
    </tbody></table></div></div>`;
    document.body.appendChild(modal);
    modal.addEventListener('click', (e) => { if (e.target.closest('[data-action="close-modal"]') || e.target === modal) closeModal(); });
...
function exportCurrentWeekCSV() {
    const dataToExport = state.data.weekOrders.map(o => ({
        'Дата': formatDate(o.createdAt), 'Мастер': o.masterName, 'Авто': o.carModel, 'Описание': o.description, 'Сумма': o.amount, 'Оплата': o.paymentType
    }));
